...
config = {
    ...
    "lora_rank": 8,
    "lora_alpha": 16.0,
    "architecture": "FedLoRA_CNN"
}

device = torch.device("cuda" if torch.cuda.is_available() else "mps" if torch.backends.mps.is_available() else "cpu")

"""FedLoRA training script."""
...
def inject_lora(model, rank, alpha):
    """Convert linear modules to LoRA-enabled versions; freeze original weights."""
...
def get_non_iid_dataloaders(num_clients, batch_size):
    ...
    # non-iid split by label among clients
...
def get_trainable_state_dict(model):
    """Return a dict of parameters with `requires_grad=True` (LoRA adapters)."""
...
def client_update(global_model, client_loader, epochs, lr):
    """Perform client-side update, returning only LoRA weights."""
...
def server_aggregate(global_model, client_weights_list):
    """Average client updates and apply them to the global model."""
    global_state_dict = global_model.state_dict()
    
    # prepare aggregation tensor
    aggregated_weights = {k: torch.zeros_like(v, dtype=torch.float32) for k, v in client_weights_list[0].items()}
    
    # sum client updates
    for weights in client_weights_list:
        for k in weights.keys():
            aggregated_weights[k] += weights[k]
            
    # compute mean
    num_clients = len(client_weights_list)
    for k in aggregated_weights.keys():
        aggregated_weights[k] = torch.div(aggregated_weights[k], num_clients)
        
    # load averaged LoRA weights into global model
